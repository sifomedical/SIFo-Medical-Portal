...
export default function ProcessCard({ process }: Props) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col bg-white rounded-xl border border-gray-200 hover:border-gray-300 hover:shadow-md transition-all duration-200 overflow-hidden"
    >
      {/* Top-Farb-Accent */}
      <div
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-gray-900 group-hover:text-[#1B3A6B] transition-colors leading-tight">
              {process.title}
            </h3>
            <p className="text-xs text-gray-500 mt-0.5">{process.subtitle}</p>
          </div>
        </div>

        {/* Description */}
        <p className="text-sm text-gray-600 leading-relaxed flex-1 line-clamp-2 mb-4">
          {process.description}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          ...
          {process.tools.length > 3 && (
            <span className="inline-block text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">
              +{process.tools.length - 3}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <span className="flex items-center gap-1 text-xs font-semibold text-[#1B3A6B] opacity-0 group-hover:opacity-100 transition-opacity">
